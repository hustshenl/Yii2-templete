...
class MenuHelper
{
    const CACHE_TAG = 'hust.shenl.menu';
    ...
    public static function getAssignedMenu($userId, $root = null, $callback = null, $refresh = false)
    {
        ...
        if ($refresh || $cache === null || ($assigned = $cache->get($key)) === false) {
            ...
            if ($cache !== null) {
                $cache->set($key, $assigned, self::$cacheDuration, new TagDependency([
                    'tags' => self::CACHE_TAG
                ]));
            }
        }
        ...
        if ($refresh || $callback !== null || $cache === null || (($result = $cache->get($key)) === false)) {
            ...
            if ($cache !== null && $callback === null) {
                $cache->set($key, $result, self::$cacheDuration, new TagDependency([
                    'tags' => self::CACHE_TAG
                ]));
            }
        }
    ...
    /**
     * Normalize menu
     * @param  array $assigned 允许使用的菜单ID
